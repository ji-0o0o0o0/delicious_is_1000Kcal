...
> 시작프로그램 폴더에 bat 파일을 등록하는 방식이 더 간단하고 안정적이었습니다.

1. `Win + R` → `shell:startup` 입력 → 시작프로그램 폴더 열기
2. 아래 내용으로 `kakaotracker.bat` 파일 생성:

```bat
@echo off
timeout /t 30 /nobreak
cd C:\프로젝트경로\build\libs
start javaw -jar delicious_is_1000Kcal-1.0-SNAPSHOT.jar
```

3. PC 재시작 시 자동으로 UI + 스케줄러 실행

> **주의**: 노트북 덮개를 닫고 사용하는 경우 **제어판 → 전원 옵션 → 덮개를 닫을 때 → 아무것도 안 함** 으로 설정하세요.
> `timeout /t 30` 은 시스템 시작 후 30초 대기 후 실행하는 설정입니다. 필요에 따라 조절하세요.